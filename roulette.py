...
def build_wheel():
  ...
  wheel = pd.DataFrame(
    ...
    # add the probobilities of landing on each slot
...
def generate_report(results, full_report=False):
  ...
  if full_report:
    print(f"Number of times each number landed: {results['number'].value_counts()[results['number'].value_counts() > 1]}")
...
def calculate_winnings(results, bet, count, color=None, number=None):
  ...
  color_winnings = 0
  number_winnings = 0
  total_amoun_bet = count * bet

  if color == "red":
    color_winnings = (
      results[results["color"] == "red"]["slot"].count() * (bet * 2) - total_amoun_bet
    )

  elif color == "black":
    color_winnings = (
      results[results["color"] == "black"]["slot"].count() * (bet * 2) - total_amoun_bet
    )

  elif color == "green":
    color_winnings = (
      results[results["color"] == "green"]["slot"].count() * (bet * 17) - total_amoun_bet
    )

  if number:
    number_winnings = (
      results[results["number"] == number]["slot"].count() * (bet * 35) - total_amoun_bet
    )
...
def print_wheel(results):
  ...
  for _, row in results.iterrows():
    if row["color"] == "red":
      ...
      #if the number 37 replate it with 00 and dont print number
      if row["slot"] == 37:
...
def spin_option(count, bet, number_bet, color):
  ...
  if not number_bet and not color:
    print("Not bet selected")

  else:
    print(f"Selected bet amout {bet}")
